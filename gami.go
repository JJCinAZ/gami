...
func (client *AMIClient) AsyncAction(action string, params Params) (<-chan *AMIResponse, error) {
	...
	if _, ok := params["ActionID"]; !ok {
		params["ActionID"] = uuid.NewV4().String()
	}
...
func newEvent(data map[string]string) (*AMIEvent, error) {
	...
	ev := &AMIEvent{ID: e, Privilege: strings.Split(data["Privilege"], ","), Params: make(map[string]string)}
	for k, v := range data {
...
func (client *AMIClient) NewConn() (err error) {
	...
	lineBytes, _, err := client.bufReader.ReadLine()
	if err != nil {
		return err
	}

	if strings.Contains(string(lineBytes), "Asterisk Call Manager") != true {
		return ErrNotAMI
	}
